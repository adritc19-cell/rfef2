...
export default async function handler(req, res) {
  ...
  const { target, selector, click, wait = 'table', html = '1', hint = 'PTS' } = req.query;

  const defaultUrl = 'https://www.rfef.es/es/competiciones/segunda-federacion';
  const url = target || defaultUrl;

  // Pack oficial (incluye libnss3 y dependencias) — evita el error de libs faltantes
  ...
  const logs = [];
  try {
    // ¡Importante! executablePath es función y le pasamos la URL del pack
    ...
    // Clicks por texto visible, separados por |
    if (click) {
