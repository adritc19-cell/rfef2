...
/**
 * Serverless function for Vercel that renders the RFEF Segunda Federación (Grupo 5) table
 * and returns ONLY the table/container.
 *
 * Query params:
 * - target   : URL to visit (default: RFEF Segunda Federación landing)
 * - click    : pipe-separated visible texts to click before capturing (e.g. "grupo 5|clasificación")
 * - selector : CSS selector for the node to return (fallback: first <table> containing "hint")
 * - hint     : text to look for inside the table (default: "PTS")
 * - wait     : selector to wait for (default: "table")
 * - html     : "1" => returns raw HTML; "0" (default) => returns JSON { html }
 */
...
export default async function handler(req, res) {
  ...
  const logs = [];
  try {
    const executablePath = await chromium.executablePath(); // IMPORTANT: function call

    // Optional: reduce cold starts in some environments
...
.standings-table img{width:22px;height:22px;border-radius:4px;vertical-align:middle}
    ...
    res.setHeader('Cache-Control', 's-maxage=900, stale-while-revalidate=86400'); // 15 min
    if (html === '1') {
